package at.fhtw.swen3.services.validation;

public  final class TestValuesRecipient {
    private TestValuesRecipient(){};

    public static final String POSTAL_CODE_CORRECT = "A-1202";
    public static final String POSTAL_CODE_WRONG = "A-120";
    public static final String COUNTRY_AUSTRIA = "Austria";
    public static final String COUNTRY_OESTERREICH = "Österreich";
    public static final String CITY_CORRECT = "Deutsch Schützen-Eisenberg";
    public static final String CITY_WRONG = "deutsch Schützen Eisenberg";



}
